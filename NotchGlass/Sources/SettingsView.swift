...
extension Notification.Name {
    ...
    /// Posted by the model-detail card's first-party pill so Settings opens on
    /// that provider's pane (Notchi → Model, with the balance in view).
    static let openProviderSettingsRequested = Notification.Name("openProviderSettingsRequested")
